    ...
    const archives = await Archive.findAll({
      attributes: [
        ...
        [
          Sequelize.fn(
            "COUNT",
            Sequelize.fn("DISTINCT", Sequelize.col("issue"))
          ),
          "total_issues",
        ],
      ],
  ...
  try {
    const archives = await Archive.findAll({
      attributes: [
        ...
        "volume",
        "issue",
        [Sequelize.fn("COUNT", Sequelize.col("issue")), "articles"],
        [Sequelize.fn("MIN", Sequelize.col("creation")), "creation"],
      ],
  ...
  try {
    ...
    let result = issue.map((article) => {
      if (
        article.file &&
        fs.existsSync(
          `${config.archivesDir}/${article.year}/vol${article.volume}issue${article.issue}/${article.file}`
        )
      ) {
        return { ...article };
      } else {
...
const archivesByRef = async (req, res) => {
  try {
    ...
    if (article.file && !fs.existsSync(path)) article.file = "";
